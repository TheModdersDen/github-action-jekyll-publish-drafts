		...
		return new (P || (P = Promise))(function (resolve, reject) {
			...
			function step(result) {
				result.done
					...
					: adopt(result.value).catch(fulfilled, rejected);
			}
...
var __asyncValues =
	...
	function (o) {
		...
		function settle(resolve, reject, d, v) {
			Promise.resolve(v).catch(function (v) {
	...
	function (thisArg, _arguments, generator) {
		...
		function step(r) {
			r.value instanceof __await
				? Promise.resolve(r.value.v).catch(fulfill, reject)
				: settle(q[0][2], r);
		}
...
function main() {
	...
	return __awaiter(this, void 0, void 0, function* () {
		...
		const GITHUB_REPOSITORY =
			env.GITHUB_REPOSITORY || env.INPUT_GITHUB_REPOSITORY;
		const GITHUB_ACTOR = env.GITHUB_ACTOR || env.INPUT_GITHUB_ACTOR;
		const INPUT_BRANCH = env.BRANCH || env.INPUT_BRANCH || "production";
		const GITHUB_TOKEN = env.GITHUB_TOKEN || env.INPUT_GITHUB_TOKEN;
		const GIT_USERNAME =
			env.GIT_USERNAME || env.INPUT_GIT_USERNAME || "github-action";
		const GIT_EMAIL =
			env.GIT_EMAIL || env.INPUT_GIT_EMAIL || "nobody@localhost";
		const GIT_MESSAGE =
			env.GIT_MESSAGE || env.INPUT_GIT_MESSAGE || "Publish drafts";
		//console.warn(env);
		...
		let fileCount = 0;
		try {
            if (!(yield fs_1.promises.stat(`${folder}/_drafts`)).isDirectory()) {
                console.warn(`No _drafts folder found!`);
                return;
            } else {
                console.warn(`Found _drafts folder! Continuing...`);
            }
			for (
				var _b = __asyncValues(walk(`${folder}/_drafts`)), _c;
				(_c = yield _b.next()), !_c.done;

			) {
				const file = _c.value;
				if ((yield fs_1.promises.stat(file)).isDirectory()) {
                    // Verify the "_drafts" folder contains markdown files and if it does, continue
                    continue;
                } else if (file.indexOf(".md") < 0) {
                    // Verify the "_drafts" folder contains markdown files and if it doesn't, exit early
                    console.warn(`No markdown files found in _drafts folder!`);
                    return;
                } else {
                    // If the file is a markdown file, continue (this is the expected behavior)
                    console.warn(`Found file: ${file}`);
                }
				fileCount++;
			...
			try {
                ...
                console.warn(`Error while processing drafts: ${e_2}`);
            }
		}
